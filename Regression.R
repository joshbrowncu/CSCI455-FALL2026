#Author: Brown, Date 08/25/2026, Purpose: Test the regression analysis

#Import dummy dataset
training_data <- mtcars

#Plot the data as a scatter plot
scatter.smooth(x=training_data$disp,
y=training_data$wt,main="SpeedVSdistance")
...
regression_model<-lm(mpg~wt,
data=training_dataset)

#Predict the relationship between mpg and the weight of the car on the test 
dataset using our generated regression model

prediction_result <- predict(regression_model, test_dataset)

#Resuts of prediction
actual_prediction_values <-
data.frame(cbind(actuals=training_data$mpg,
predicteds=prediction_result))
actual_prediction_values
